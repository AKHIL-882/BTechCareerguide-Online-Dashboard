const DropDownCheckBox = ({ label, options, selectedValues, onChange, showDropdown, toggleDropdown }) => {
    return (
      <div className="relative">
        ...
        <div className="relative">
          <button type="button" onClick={toggleDropdown} className="w-full text-left py-2 px-4 bg-white border rounded-md">
            {selectedValues.length === 0 ? "Select" : selectedValues.join(", ")}
          </button>
          {showDropdown && (
            <div className="absolute z-10 w-full bg-white border rounded-md shadow-lg mt-1">
              {options.map((option) => (
                <label key={option.value} className="block py-2 px-4 hover:bg-gray-200 cursor-pointer">
                  <input
                    type="checkbox"
