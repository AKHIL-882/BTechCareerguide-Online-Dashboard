...
import { get as apiGet } from "../../api/apiConfig";
...
const JobFilters = ({ setFilteredJobs, filteredJobs }) => {
  ...
  const fetchDropdownData = useCallback(async () => {
    try {
      const response = await apiGet("/standard-data");
  ...
  const fetchFilteredJobs = useCallback(async () => {
    const shouldFetch = isFilterSelected || activeCategory !== "All";
    if (!shouldFetch) return;

    setLoading(true);
    try {
      const response = await apiGet("/jobs/filter", {
        params: {
