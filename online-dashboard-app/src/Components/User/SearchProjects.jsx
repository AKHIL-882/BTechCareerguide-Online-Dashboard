...
const SearchProjects = ({ setProjects }) => {
  ...
  const handleSearchChange = (e) => {
    ...
    searchProject(value, setProjects); // Call search function on input change
  };
  return (
    <div
      className="flex items-center border border-gray-300 rounded-full p-2 transition-all duration-300 overflow-hidden w-full"
    >
      <FaSearch
        className="text-violet-500 text-lg cursor-pointer flex-shrink-0"
      />
      <input
        type="text"
      ...
      />
      {error && <div className="text-red-500 text-sm">{error}</div>} {/* Optional: Display error message */}
    </div>
  );
};
